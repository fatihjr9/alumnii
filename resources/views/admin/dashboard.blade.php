                 ...
                 @foreach($galeri as $item)
                    <div id="default-carousel" class="relative w-full" data-carousel="slide">
                        ...
                        <div class="relative h-56 overflow-hidden rounded-lg md:h-96">
                            @if(is_array(json_decode($item->galeri, true)))
                                        @foreach(json_decode($item->galeri, true) as $index => $image)
                                            <div class="hidden duration-700 ease-in-out" data-carousel-item>
                                                <img src="{{ asset('galeri/' . $image) }}" class="w-full object-cover" alt="Gallery Image {{ $index + 1 }}">
                                            </div>
                                        @endforeach
                                    @endif
                        </div>
            ...
            <div class="flex flex-row items-start justify-between gap-8">
                <div class="flex flex-col gap-4 w-full">
                    <div class="w-full space-y-4">
                        <div class="p-4 bg-white rounded-md">
                            <div class="flex flex-row items-center justify-between mb-3">
                                ...
                                <a href="{{ route('berita.index') }}" class="text-sm">Lihat Semua</a>
                            </div>
                    ...
                    <div class="p-4 bg-white rounded-md">
                        ...
                        <a href="{{ route('agenda.index') }}" class="w-full flex bg-slate-100 hover:bg-indigo-600 hover:text-white hover:transition-all py-2 justify-center rounded-md mt-4">Lihat Semua</a>
                    </div>
                </div>
                {{--  --}}
                    ...
                    <div class="p-4 bg-white rounded-md">
                        ...
                        @foreach($fakultas as $item)
                            <div class="border-b pb-1">
                                <h5 class="font-medium">{{ $item->nama_fakultas }}</h5>
                            </div>
                        @endforeach
                    </div>
                    <div class="p-4 bg-white rounded-md">
                        ...
                        @endforeach
                        <a href="{{ route('alumni.index') }}" class="w-full flex bg-slate-100 hover:bg-indigo-600 hover:text-white hover:transition-all py-2 justify-center rounded-md mt-2">Lihat Semua</a>
                    </div>
                    <div class="p-4 bg-white rounded-md">
